...
def to_excel(tryp):
    ...
    excel = Excel()
    excel.write_index(ws, tryp)
    excel.write_columns(ws, tryp)
    excel.write_values_labels(ws, tryp)
    excel.write_values(ws, tryp)
    wb.save(filename)


class Excel(object):
    def merge_labels(self, indexes, index_width, total_width):
        labels = {}
    ...
    def write_columns(self, ws, tryp):
        rows = tryp.rows
    ...
    def write_values_labels(self, ws, tryp):
        rows = tryp.rows
        columns = tryp.columns
        labels = tryp.labels
